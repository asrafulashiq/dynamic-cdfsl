...
class LearningRateMonitor(Callback):
        ...
        if self.log_momentum:
            ...
            def _check_no_key(key):
                return any(key not in sch['scheduler'].optimizer.defaults
                           for sch in trainer.lr_schedulers)
    ...
    def on_train_batch_start(self, trainer, pl_module, *args, **kwargs):
        ...
        if self.logging_interval != 'epoch':
            ...
            if latest_stat:
                pl_module.log_dict(latest_stat, on_step=True)
    ...
    def on_train_epoch_start(self, trainer, pl_module, *args, **kwargs):
        ...
        if self.logging_interval != 'step':
            ...
            if latest_stat:
                pl_module.log_dict(latest_stat, on_epoch=True)
        ...
        for name, scheduler in zip(self.lr_sch_names, trainer.lr_schedulers):
            if scheduler['interval'] == interval or interval == 'any':
                ...
                for i, pg in enumerate(param_groups):
                    ...
                    lr = self._extract_lr(param_group=pg,
                                          name=f'{name}{suffix}')
                    latest_stat.update(lr)
                    momentum = self._extract_momentum(
                        param_group=pg,
                        name=f'{name}-momentum{suffix}',
                        use_betas=use_betas)
    ...
    def _extract_momentum(self, param_group, name: str,
                          use_betas: bool) -> Dict[str, float]:
        if not self.log_momentum:
            return {}

        momentum = param_group.get(
            'betas')[0] if use_betas else param_group.get('momentum', 0)
    ...
    def _should_log(trainer) -> bool:
        should_log = ((trainer.global_step + 1) % trainer.log_every_n_steps
                      == 0 or trainer.should_stop)
